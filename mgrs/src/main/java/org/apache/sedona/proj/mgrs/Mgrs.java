 ...
 *
 * <p>This class provides methods to convert between WGS84 lat/lng coordinates and MGRS coordinates,
 * ported from the JavaScript mgrs library.
 *
...
public class Mgrs {
  ...
  public static String forward(double lon, double lat, int accuracy) {
    ...
    if (lat < -80 || lat > 84) {
      throw new IllegalArgumentException(
          "Latitude " + lat + " is outside MGRS limits (80°S to 84°N)");
    }
  ...
  private static UtmCoordinate decode(String mgrsString) {
    ...
    if (zoneLetter <= 'A'
        ...
        || zoneLetter == 'O') {
      throw new IllegalArgumentException(
          "MGRS zone letter " + zoneLetter + " not handled: " + mgrsString);
    }
    ...
    if (remainder % 2 != 0) {
      throw new IllegalArgumentException(
          "MGRS has to have an even number of digits after the zone letter and two 100km letters - front half for easting meters, second half for northing meters "
              + mgrsString);
    }
   ...
   * zone number, figure out the easting value.
   */
   ...
   * zone number, figure out the northing value.
   */
  ...
  /**
   * The function getMinNorthing returns the minimum northing value of a MGRS zone. Ported from
   * Geotrans' c Latitude_Band_Value structure table.
   */
