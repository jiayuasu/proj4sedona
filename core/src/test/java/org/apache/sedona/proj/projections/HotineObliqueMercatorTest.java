...
/**
 * Test cases for Hotine Oblique Mercator projection. Tests both forward and inverse transformations
 * with known coordinate pairs.
 */
  ...
  public void testEdgeCases() {
    ...
    for (Point input : edgeCases) {
      ...
      if (Math.abs(input.y) < Math.PI / 2 - 0.1) { // Skip north pole
        assertFalse(
            Double.isNaN(projected.x) || Double.isNaN(projected.y),
