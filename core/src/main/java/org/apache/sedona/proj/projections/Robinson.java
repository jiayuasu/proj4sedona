...
/**
 * Robinson projection implementation. Ported from proj4js robin.js
 *
 * <p>The Robinson projection is a compromise pseudocylindrical map projection widely used for world
 * maps. It was specifically created in an attempt to find a good compromise to the problem of
 * readily showing the whole globe as a flat image.
 *
 * <p>Based on https://github.com/OSGeo/proj.4/blob/master/src/PJ_robin.c Polynomial coefficients
 * from http://article.gmane.org/gmane.comp.gis.proj-4.devel/6039
 */
public class Robinson {
  ...
  private static double poly3Val(double[] coefs, double x) {
    return coefs[0] + x * (coefs[1] + x * (coefs[2] + x * coefs[3]));
  }

  private static double poly3Der(double[] coefs, double x) {
    return coefs[1] + x * (2 * coefs[2] + x * 3 * coefs[3]);
  }

  private static double newtonRaphson(
      java.util.function.Function<Double, Double> fDf, double start, double maxErr, int iters) {
    double x = start;
    for (int i = 0; i < iters; i++) {
      double upd = fDf.apply(x);
      x -= upd;
      if (Math.abs(upd) < maxErr) {
        break;
      }
    }
    return x;
  ...
  /**
   * Initialize the Robinson projection parameters.
   *
   * @param projection the projection to initialize
   */
  public static void init(Projection projection) {
    projection.es = 0;
    if (projection.title == null || projection.title.isEmpty()) {
      projection.title = "Robinson";
    }
  }
  ...
  public static Point forward(Projection projection, Point point) {
    double lon = MathUtils.adjustLon(point.x - projection.long0);
    double dphi = Math.abs(point.y);
    ...
    point.x = x * projection.a * FXC + projection.x0;
    point.y = y * projection.a * FYC + projection.y0;
    return point;
  }
  ...
  public static Point inverse(Projection projection, Point point) {
    double x = (point.x - projection.x0) / (projection.a * FXC);
    double y = Math.abs(point.y - projection.y0) / (projection.a * FYC);

    if (y >= 1) {
      // Pathologic case
      x /= COEFS_X[NODES][0];
      y = point.y < 0 ? -Values.HALF_PI : Values.HALF_PI;
    } else {
      // Find table interval
      ...
      final double[] coefs = COEFS_Y[i];
      final double yy = y;
      double t = 5 * (y - coefs[0]) / (COEFS_Y[i + 1][0] - coefs[0]);

      // Find t so that poly3_val(coefs, t) = y
      t =
          newtonRaphson(
              xx -> (poly3Val(coefs, xx) - yy) / poly3Der(coefs, xx), t, Values.EPSLN, 100);

      x /= poly3Val(COEFS_X[i], t);
      y = (5 * i + t) * Values.D2R;
      if (point.y < 0) {
        y = -y;
      }
  ...
  /** Projection names */
  public static final String[] NAMES = {"Robinson", "robin"};
}
