...
/**
 * Parser for PROJJSON format coordinate reference system definitions. Converts PROJJSON definitions
 * to PROJ strings and Projection objects.
 */
public class ProjJsonParser {
  ...
  private static void addEllipsoidParameters(
      StringBuilder proj, ProjJsonDefinition.Ellipsoid ellipsoid) {
  ...
  private static void addConversionParameters(StringBuilder proj, ProjJsonDefinition definition) {
    ...
    for (ProjJsonDefinition.Conversion.Parameter param :
        definition.getConversion().getParameters()) {
  ...
  /**
   * Create a PROJJSON definition from a PROJ string. This is a basic implementation that creates a
   * simple PROJJSON structure.
   *
