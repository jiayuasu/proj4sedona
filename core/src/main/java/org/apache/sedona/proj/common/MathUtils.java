...
/**
 * Mathematical utility functions for coordinate transformations. These functions are ported from
 * the JavaScript common utilities.
 */
public final class MathUtils {
  ...
  /**
   * Adjusts longitude to valid range [-PI, PI]. Ported from adjust_lon.js
   *
  ...
  /**
   * Returns the sign of a number. Ported from sign.js
   *
   * @param x the number
  ...
  /**
   * Calculates the meridian scale factor. Ported from msfnz.js
   *
  ...
  /**
   * Calculates the conformal latitude. Ported from tsfnz.js
   *
   * @param eccent eccentricity
  ...
  /**
   * Converts conformal latitude to geodetic latitude. Ported from phi2z.js
   *
   * @param eccent eccentricity
  ...
  public static double qsfnz(double e, double sinphi) {
    double con = e * sinphi;
    return (1 - e * e)
        * (sinphi / (1 - con * con) - (1 / (2 * e)) * Math.log((1 - con) / (1 + con)));
  }
   ...
   */
  public static double[] vincenty(
      double a, double e, double lat1, double lon1, double az, double s) {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      for (int j = 0; j < 20; j++) {
        ...
        double deltaSigmaNew =
            (b * (1 + f * f / 4 * (1 - Math.cos(2 * sigma1))) * sigma - s)
    ...
    double lat2 =
        Math.atan2(
            Math.sin(u1) * Math.cos(sigma) + Math.cos(u1) * Math.sin(sigma) * Math.cos(az),
            (1 - f)
                * Math.sqrt(
                    Math.sin(alpha) * Math.sin(alpha)
                        + Math.pow(
                            Math.sin(u1) * Math.sin(sigma)
                                - Math.cos(u1) * Math.cos(sigma) * Math.cos(az),
                            2)));

    double lambda =
        Math.atan2(
            Math.sin(sigma) * Math.sin(az),
        ...
        lon1
            ...
            - (1 - c)
                ...
                * (sigma
                    + c
                        * Math.sin(sigma)
                        * (Math.cos(2 * sigma1 + sigma)
                            + c
                                * Math.cos(sigma)
                                * (-1 + 2 * Math.pow(Math.cos(2 * sigma1 + sigma), 2))));

    double az2 =
        Math.atan2(
            Math.sin(alpha),
            -Math.sin(u1) * Math.sin(sigma) + Math.cos(u1) * Math.cos(sigma) * Math.cos(az));
  ...
  /**
   * Adjusts UTM zone based on longitude. Ported from adjust_zone.js
   *
  ...
  /**
   * Calculates the Gauss-Krüger transverse Mercator projection. Ported from gatg.js
   *
  ...
  /**
   * Calculates the radius of curvature in the prime vertical. Ported from gN.js
   *
   * @param a semi-major axis
   * @param e eccentricity
   * @param sinphi sine of latitude
  ...
  /**
   * Calculates the fL function for transverse Mercator projections. Ported from fL.js
   *
   * @param x the x value
   * @param L the L value
   * @return result
  ...
  /**
   * Calculates the srat function for ellipsoid calculations. Ported from srat.js
   *
  ...
  /**
   * Arc sine function that handles values outside [-1, 1] range. Ported from asinz.js
   *
   * @param x the value
  ...
  /**
   * Adjusts latitude to valid range [-PI/2, PI/2]. Ported from adjust_lat.js
   *
