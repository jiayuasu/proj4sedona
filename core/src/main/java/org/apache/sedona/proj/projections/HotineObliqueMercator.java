...
/**
 * Hotine Oblique Mercator projection implementation. Ported from proj4js/lib/projections/omerc.js
 */
  ...
  public void initialize(Projection proj) {
    ...
    if (alp || gam) {
      ...
      if (Math.abs(phi1 - phi2) <= TOL
          ...
          || Math.abs(Math.abs(proj.lat0) - Values.HALF_PI) <= TOL
    ...
    if (alp || gam) {
      ...
      this.lam0 =
          MathUtils.adjustLon(
              0.5 * (lam1 + lam2)
                  - Math.atan(J * Math.tan(0.5 * this.B * (lam1 - lam2)) / p) / this.B);
      gamma0 =
          Math.atan(2 * Math.sin(this.B * MathUtils.adjustLon(lam1 - this.lam0)) / (F - 1 / F));
