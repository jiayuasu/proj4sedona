...
/**
 * Lightweight GeoTIFF reader for PROJ datum grids. This implementation uses Java's built-in
 * javax.imageio package and custom GeoTIFF metadata parsing to avoid heavy dependencies.
 */
public class GeoTiffReader {
  ...
  public static double[] interpolateGrid(GeoTiffGrid grid, double lat, double lon) {
    ...
    for (GeoTiffSubgrid subgrid : grid.subgrids) {
      if (latDeg >= subgrid.minLat
          && latDeg <= subgrid.maxLat
          && lonDeg >= subgrid.minLon
          && lonDeg <= subgrid.maxLon) {
