...
/**
 * Grid-based datum shift utilities for NTv2 and other grid formats.
 * This class implements grid-based datum adjustments similar to the JavaScript implementation.
 */
public class GridShift {
    ...
    public static class GridFile {
        ...
        public GridFile(String name, double minLat, double maxLat, double minLon, double maxLon,
                       double latStep, double lonStep, int latCount, int lonCount,
    ...
    public static double[] interpolateGrid(GridFile grid, double lat, double lon) {
        ...
        if (latDeg < grid.minLat || latDeg > grid.maxLat ||
            lonDeg < grid.minLon || lonDeg > grid.maxLon) {
