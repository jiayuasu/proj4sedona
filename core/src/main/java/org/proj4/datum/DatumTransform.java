...
/**
 * Datum transformation utilities for converting between different geodetic datums.
 * This class implements the same functionality as the JavaScript datum_transform.js module.
 */
public class DatumTransform {
    ...
    public static Point transform(Datum.DatumDef source, Datum.DatumDef dest, Point point) {
        ...
        if (source_es == dest_es && source_a == dest_a && 
            !checkParams(source.datumType) && !checkParams(dest.datumType)) {
            return point;
        }
