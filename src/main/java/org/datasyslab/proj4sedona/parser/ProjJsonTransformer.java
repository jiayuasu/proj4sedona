...
public final class ProjJsonTransformer {
    ...
    public static ProjectionDef transform(Map<String, Object> projjson) {
        ...
        // Coordinate-system maps may be visited before conversion parameters because
        // callers are free to use unordered maps. Derive the polar operational axis
        // only after every top-level field and projection default has been applied.
        applyPolarAxisMapping(projjson, def);
    ...
    @SuppressWarnings("unchecked")
    private static void applyPolarAxisMapping(
            Map<String, Object> projjson, ProjectionDef def) {
        Object coordSys = projjson.get("coordinate_system");
        if (!(coordSys instanceof Map)) {
            return;
        }
        Object axisList = ((Map<String, Object>) coordSys).get("axis");
        if (!(axisList instanceof List)) {
            return;
        }
        String roleMapping = polarAxisRoleMapping(
            (List<Map<String, Object>>) axisList, def.getAxis(), def);
        if (roleMapping != null) {
            def.setAxisMapping(roleMapping);
        }
    }

    private static String polarAxisRoleMapping(
            List<Map<String, Object>> axes, String directionAxis, ProjectionDef def) {
        if (axes.size() != 2 || directionAxis == null || directionAxis.length() != 3
                || directionAxis.charAt(0) != directionAxis.charAt(1)
                || (directionAxis.charAt(0) != 'n' && directionAxis.charAt(0) != 's')) {
            return null;
        }

        String first = mapProjectedAxisRole(axes.get(0));
        String second = mapProjectedAxisRole(axes.get(1));
        if (first == null || second == null || first.equals(second)) {
            return null;
        }
        if (!(("e".equals(first) && "n".equals(second))
                || ("n".equals(first) && "e".equals(second)))) {
            return null;
        }

        Double latitudeOfOrigin = def.getLat0();
        if (latitudeOfOrigin == null
                || Math.abs(Math.abs(latitudeOfOrigin) - Values.HALF_PI) > 1e-9) {
            return null;
        }
        boolean northPole = latitudeOfOrigin > 0;
        char expectedDirection = northPole ? 's' : 'n';
        if (directionAxis.charAt(0) != expectedDirection) {
            return null;
        }

        Double firstMeridian = axisMeridianRadians(axes.get(0));
        Double secondMeridian = axisMeridianRadians(axes.get(1));
        if (firstMeridian == null || secondMeridian == null) {
            return null;
        }
        double longitudeOfOrigin = def.getLong0() != null ? def.getLong0() : 0.0;
        if (!matchesPolarMeridian(first, firstMeridian, northPole, longitudeOfOrigin)
                || !matchesPolarMeridian(
                    second, secondMeridian, northPole, longitudeOfOrigin)) {
            return null;
        }
    ...
    @SuppressWarnings("unchecked")
    private static Double axisMeridianRadians(Map<String, Object> axis) {
        Object meridianObject = axis.get("meridian");
        if (!(meridianObject instanceof Map)) {
            return null;
        }
        Map<String, Object> meridian = (Map<String, Object>) meridianObject;
        Object longitude = meridian.get("longitude");
        Object unit = meridian.get("unit");
        if (longitude instanceof Map) {
            Map<String, Object> longitudeMap = (Map<String, Object>) longitude;
            unit = longitudeMap.get("unit");
            longitude = longitudeMap.get("value");
        }

        Double value = finiteDouble(longitude);
        if (value == null) {
            return null;
        }
        double factor = Values.D2R;
        if (unit instanceof Map) {
            Double conversionFactor = finiteDouble(
            ...
            String unitName = unit.toString().trim().toLowerCase();
            if ("radian".equals(unitName) || "radians".equals(unitName)) {
                factor = 1.0;
            } else if ("grad".equals(unitName) || "gon".equals(unitName)) {
                factor = Math.PI / 200.0;
            } else if (!("degree".equals(unitName) || "degrees".equals(unitName))) {
                return null;
            }
        }
        double radians = value * factor;
        return Double.isFinite(radians) ? radians : null;
    }
    ...
    private static boolean matchesPolarMeridian(
            String role, double actual, boolean northPole, double longitudeOfOrigin) {
        double expected;
        if ("e".equals(role)) {
            expected = longitudeOfOrigin + Values.HALF_PI;
        } else {
            expected = longitudeOfOrigin + (northPole ? Math.PI : 0.0);
        }
        return Math.abs(Math.IEEEremainder(actual - expected, 2 * Math.PI)) <= 1e-9;
    }

    private static String mapProjectedAxisRole(Map<String, Object> axis) {
        String nameRole = null;
        Object name = axis.get("name");
        if (name != null) {
            String normalized = name.toString().toLowerCase();
            if (normalized.contains("easting")) {
                nameRole = "e";
            } else if (normalized.contains("northing")) {
                nameRole = "n";
            }
        }

        String abbreviationRole = null;
        Object abbreviation = axis.get("abbreviation");
        if (abbreviation != null) {
            String normalized = abbreviation.toString().trim().toLowerCase();
            if ("e".equals(normalized) || "x".equals(normalized)) {
                abbreviationRole = "e";
            } else if ("n".equals(normalized) || "y".equals(normalized)) {
                abbreviationRole = "n";
            }
        }
        if (nameRole != null && abbreviationRole != null
                && !nameRole.equals(abbreviationRole)) {
            return null;
        }
        return nameRole != null ? nameRole : abbreviationRole;
    }
